 ...
 *
 * The delete button is a second form, and it only exists for an empty section:
 * the foreign key is ON DELETE SET NULL, so deleting one that still holds
 * products would take every one of them out of the catalogue without a word.
 * The action re-checks that; this only hides a button that could do nothing.
 *
 * Two presentations, one set of wiring. `variant="card"` is what a phone gets —
 * the table is 900px wide, so on a 400px screen the number, the name and the
 * sort field were all off to the right of the section they belong to. Both
 * variants render the same two forms, so a change to either reaches both.
 */
export function CatalogSectionRow({
  section,
  variant = "row",
}: {
  ...
  const editForm = (
    <form action={formAction} className="flex flex-wrap items-center gap-2">
      ...
      <input
        ...
        className={`${cell} ${card ? "min-w-0 flex-1" : "w-56"}`}
      />
      <input
        name="sort"
        type="number"
  ...
  const deleteForm = section.productCount === 0 && (
  ...
  if (card) {
    return (
      <li className="p-3">
        ...
        <p className="mt-1 text-sm tabular-nums text-ink-500">{counts}</p>
  ...
  return (
    <tr className="border-b border-ink-900/4 align-middle last:border-0">
      ...
      <td className="px-3 py-2 text-sm tabular-nums text-ink-500">{counts}</td>
      <td className="px-3 py-2" colSpan={3}>
        {editForm}
      </td>
      <td className="px-3 py-2 text-right">{deleteForm}</td>
    </tr>
  );
}
